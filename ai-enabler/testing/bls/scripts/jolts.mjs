...
const outputDir = path.join(rootDir, 'bls', 'outputs', 'jolts');
...
async function main() {
  ...
  const seriesIds = (process.env.BLS_SERIES_IDS || 'JTS000000000000000JOL,JTS000000000000000HIL,JTS000000000000000LDL,JTS000000000000000QUL,JTS000000000000000JOR')
    .split(',')
  ...
  const rawPath = path.join(outputDir, `jolts-${stamp}.json`);
  const reportPath = path.join(outputDir, `jolts-${stamp}.report.json`);
  ...
  const firstData = firstSeries?.data || [];
  ...
  await writeFile(reportPath, JSON.stringify({
    ...
    firstSeriesId: firstSeries?.seriesID || null,
    firstSeriesDataPoints: firstData.length,
