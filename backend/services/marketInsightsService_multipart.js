 ...
 */
function buildCoreDataPrompt(location) {
 ...
 */
function buildOpportunitiesPrompt(location) {
 ...
 */
function buildActionPlanPrompt(location) {
...
function validateActionPlan(data) {
...
export async function generateMarketInsights(location, userId) {
  try {
    ...
    let coreData;
    try {
      coreData = await generateWithRAG(
      ...
      logger.error('❌ Part 1/3 failed:', error.message);
    ...
    let opportunities;
    try {
      opportunities = await generateWithRAG(
      ...
      logger.error('❌ Part 2/3 failed:', error.message);
    ...
    let actionPlan;
    try {
      actionPlan = await generateWithRAG(
      ...
      logger.error('❌ Part 3/3 failed:', error.message);
      // Provide minimal fallback
      actionPlan = {
        strategies_by_profile: {
          new_graduates: [],
    ...
    if (coreData.report_sources && Array.isArray(coreData.report_sources)) {
      coreData.report_sources.forEach(source => allSources.add(source));
    }
    if (opportunities.report_sources && Array.isArray(opportunities.report_sources)) {
      opportunities.report_sources.forEach(source => allSources.add(source));
    }
    if (actionPlan.report_sources && Array.isArray(actionPlan.report_sources)) {
      actionPlan.report_sources.forEach(source => allSources.add(source));
    }
    ...
    logger.info(`📚 Total sources used: ${combinedInsights.report_sources.length}`);
