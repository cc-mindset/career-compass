...
export async function retrieveContext(queryText, options = {}) {
  ...
  try {
    ...
    // Step 1: Create embedding for the query (NO LONGER CREATING - JUST ACCESSING)
    // Since you already have embeddings in Pinecone, we'll use text search instead
    // Actually, we still need embeddings to query Pinecone's vector search
    ...
    for (const namespace of namespaces) {
      ...
      const matches = nsResults.matches
    ...
    // Cache the results
    if (useCache) {
      cache.set(cacheKey, context, cache.ttlDefaults.pineconeResults);
    }
...
export function formatContextForLLM(context) {
  ...
  if (context.namespaces['news-data']?.matches?.length > 0) {
  ...
  if (context.namespaces['bls-data']?.matches?.length > 0) {
    sections.push('\n=== BLS EMPLOYMENT DATA ===');
  ...
  if (context.namespaces['reports-data']?.matches?.length > 0) {
 ...
 */
export async function generateWithRAG(query, systemPrompt, options = {}) {
  const {
    namespaces = ['news-data', 'bls-data', 'reports-data'],
    topKPerNamespace = { 'news-data': 10, 'bls-data': 10, 'reports-data': 8 },
    responseFormat = 'json',
  ...
  try {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        lastError = error;
        logger.warn(`⚠️  RAG generation attempt ${attempt}/${maxRetries} failed:`, error.message);
    ...
    // Cache the response
    if (useCache) {
      cache.set(cacheKey, response, cache.ttlDefaults.marketInsights);
    }
