...
const ThemePage: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold mb-2 text-slate-900">Theme & Colors</h1>
      ...
      {/* Primary Colors */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-900">Primary Colors</h2>
          ...
          {Object.entries(colorPalette.indigo).map(([shade, color]) => (
            <Card key={shade} className="p-4" padding="sm">
              ...
              <div className="text-sm">
                <p className="font-bold text-slate-900">Indigo {shade}</p>
                <p className="text-slate-500 text-xs font-mono">{color}</p>
              </div>
            </Card>
          ))}
        </div>
      ...
      {/* Neutral Colors */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-900">Neutral Colors</h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Object.entries(colorPalette.slate).map(([shade, color]) => (
            <Card key={shade} className="p-4" padding="sm">
              <div
                className="w-full h-24 rounded-xl mb-3 border border-slate-200"
                style={{ backgroundColor: color }}
              />
              <div className="text-sm">
                <p className="font-bold text-slate-900">Slate {shade}</p>
                <p className="text-slate-500 text-xs font-mono">{color}</p>
              </div>
            </Card>
          ))}
      ...
      {/* Semantic Colors */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-900">Semantic Colors</h2>
          ...
          {['emerald', 'rose', 'amber'].map((colorName) => {
            ...
            return (
              <Card key={colorName} className="p-6" padding="md">
                <h3 className="text-lg font-bold mb-4 capitalize text-slate-900">{colorName}</h3>
      ...
      {/* Gradients */}
      <section>
        <h2 className="text-2xl font-bold mb-4 text-slate-900">Gradients</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {gradients.map((gradient) => (
            <Card key={gradient.name} className="p-6" padding="md">
              <h3 className="text-lg font-bold mb-3 text-slate-900">{gradient.name}</h3>
              <div className={`w-full h-32 rounded-xl ${gradient.classes}`} />
              <p className="mt-3 text-xs font-mono text-slate-500">{gradient.classes}</p>
            </Card>
          ))}
        </div>
      </section>
