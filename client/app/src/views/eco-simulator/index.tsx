...
const EcoSimulatorView: React.FC<EcoSimulatorViewProps> = ({ user }) => {
  ...
  const marketDemand = data
    ? Math.max(
        10,
        Math.min(
          100,
          calculateScore(data.career_demand_formula, baseVariables),
        ),
      )
    : 50;
  const resiliencyScore = data
    ? Math.max(
        10,
        Math.min(
          100,
          calculateScore(data.layoff_chances_formula, baseVariables),
        ),
      )
    : 50;
  const salaryIncreaseChances = data
    ? Math.max(
        10,
        Math.min(
          100,
          calculateScore(data.salary_increment_formula, baseVariables),
        ),
      )
    : 50;
  const careerAdvancementOpportunities = data
    ? Math.max(
        10,
        Math.min(
          100,
          calculateScore(
            data.career_growth_opportunities_formula,
            baseVariables,
          ),
        ),
      )
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="mb-4 md:mb-5">
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight mb-1 sm:mb-2 flex items-center gap-2 sm:gap-3 text-slate-900">
          Career Eco Simulator{" "}
          <BarChart3 className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-600 flex-shrink-0" />
        </h1>
        <p className="text-slate-500 text-sm sm:text-base">
          Model the impact of global economic variables on your role as a{" "}
          {user.role}
        </p>
        {loading && (
          <p className="text-slate-500 text-sm mt-2 flex items-center gap-2">
            <Loader2 className="w-4 h-4 animate-spin" /> Loading simulation
            data...
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        ...
        <div className="lg:col-span-1 space-y-6 md:space-y-10">
            ...
            <div className="space-y-8 md:space-y-10">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-extrabold text-cyan-600 bg-cyan-50 px-2 py-1 rounded-lg">
                    {aiTechAdvancement}%
                  </span>
                </div>
                <input
                  ...
                  onChange={(e) =>
                    setAiTechAdvancement(parseInt(e.target.value))
                  }
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <label className="text-sm font-bold text-slate-700">
                    Global Instability Levels
                  </label>
                  <span className="text-xs font-extrabold text-rose-600 bg-rose-50 px-2 py-1 rounded-lg">
                    {globalInstabilityLevels}%
                  </span>
                </div>
                <input
                  ...
                  onChange={(e) =>
                    setGlobalInstabilityLevels(parseInt(e.target.value))
                  }
          ...
          <TipCard
            text={
              data?.tips ||
              "Increasing your Upskilling Velocity is the single most effective way to counter high Global Instability Levels."
            }
            compact
          />
        </div>
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col items-center text-center">
              <div
                className={`p-3 md:p-4 rounded-2xl md:rounded-3xl ${resiliency.bg} mb-4 md:mb-6`}
              >
                <ShieldCheck
                  className={`w-6 h-6 md:w-8 md:h-8 ${resiliency.color}`}
                />
              </div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5 md:mb-2">
                Layoff Chances
              </p>
              <h3
                className={`text-3xl md:text-4xl font-extrabold ${resiliency.color} mb-1.5 md:mb-2`}
              >
                {Math.round(resiliencyScore)}%
              </h3>
              <p className="text-slate-500 text-xs sm:text-sm font-medium">
                Status: {resiliency.label}
              </p>
            </div>

            <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col items-center text-center">
              <div
                className={`p-3 md:p-4 rounded-2xl md:rounded-3xl ${demand.bg} mb-4 md:mb-6`}
              >
                <TrendingUp
                  className={`w-6 h-6 md:w-8 md:h-8 ${demand.color}`}
                />
              </div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5 md:mb-2">
                Career Demand
              ...
              <h3
                className={`text-3xl md:text-4xl font-extrabold ${demand.color} mb-1.5 md:mb-2`}
              >
            ...
            <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col items-center text-center">
              <div
                className={`p-3 md:p-4 rounded-2xl md:rounded-3xl ${salaryIncrease.bg} mb-4 md:mb-6`}
              >
                <Wallet
                  className={`w-6 h-6 md:w-8 md:h-8 ${salaryIncrease.color}`}
                />
              </div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5 md:mb-2">
                Salary Increase Chances
              </p>
              <h3
                className={`text-3xl md:text-4xl font-extrabold ${salaryIncrease.color} mb-1.5 md:mb-2`}
              >
                {Math.round(salaryIncreaseChances)}%
              </h3>
              <p className="text-slate-500 text-xs sm:text-sm font-medium">
                Status: {salaryIncrease.label}
              </p>
            </div>

            <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm flex flex-col items-center text-center">
              <div
                className={`p-3 md:p-4 rounded-2xl md:rounded-3xl ${careerAdvancement.bg} mb-4 md:mb-6`}
              >
                <BriefcaseBusiness
                  className={`w-6 h-6 md:w-8 md:h-8 ${careerAdvancement.color}`}
                />
              </div>
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5 md:mb-2">
              ...
              <h3
                className={`text-3xl md:text-4xl font-extrabold ${careerAdvancement.color} mb-1.5 md:mb-2`}
              >
                {Math.round(careerAdvancementOpportunities)}%
              </h3>
              <p className="text-slate-500 text-xs sm:text-sm font-medium">
                Status: {careerAdvancement.label}
              </p>
            </div>
          </div>

          <div className="bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm">
            <h2 className="text-lg md:text-xl font-bold mb-6 md:mb-8 text-slate-900">
              Simulation Insights
            </h2>
            <div className="space-y-5 md:space-y-6">
              {data?.simulation_insights &&
                data?.simulation_insights?.length > 0 &&
                data?.simulation_insights.map((ele, i) => (
                  <div key={i} className="flex gap-4 md:gap-6 items-start">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 mt-2.5 shrink-0 flex-shrink-0"></div>
                    <div className="min-w-0">
                      <h4 className="font-bold text-slate-900 mb-1 text-base md:text-lg">
                        {i === 0
