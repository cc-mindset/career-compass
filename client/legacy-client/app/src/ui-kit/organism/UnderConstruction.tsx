...
const UnderConstruction: React.FC<UnderConstructionProps> = ({ viewName, onBack }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] sm:min-h-[60vh] text-center px-4 sm:px-6 animate-in zoom-in-95 duration-500">
      <div className="bg-indigo-50 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] mb-6 md:mb-8">
        <Construction className="w-16 h-16 sm:w-24 sm:h-24 text-indigo-600 animate-bounce" />
      </div>
      <h2 className="text-2xl sm:text-3xl font-extrabold mb-3 md:mb-4 capitalize text-slate-900">
        {viewName.replace('-', ' ')} Coming Soon
      </h2>
      <p className="text-slate-500 max-w-md mb-6 md:mb-8 text-sm sm:text-base px-2">
