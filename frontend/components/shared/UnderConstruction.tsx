...
const UnderConstruction: React.FC<UnderConstructionProps> = ({ viewName, onBack }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] sm:min-h-[60vh] text-center px-4 sm:px-6 animate-in zoom-in-95 duration-500">
      ...
      <h2 className="text-2xl sm:text-3xl font-extrabold mb-3 md:mb-4 capitalize text-slate-900 dark:text-white">{viewName.replace('-', ' ')} Coming Soon</h2>
      ...
      <button 
        onClick={onBack}
        className="flex items-center gap-2 bg-slate-900 dark:bg-white dark:text-black text-white px-6 py-2.5 sm:px-8 sm:py-3 rounded-xl sm:rounded-2xl font-bold hover:scale-105 transition-all shadow-xl touch-manipulation"
      >
